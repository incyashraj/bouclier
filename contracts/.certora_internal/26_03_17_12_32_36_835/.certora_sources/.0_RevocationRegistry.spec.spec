...
methods {
    function revokeWithReason(bytes32, uint8, string) external;
...
rule revokeAlwaysSetsFlag {
    ...
    revokeWithReason(e, agentId, reason, notes);
...
rule doubleRevokeReverts {
    ...
    revokeWithReason@withrevert(e, agentId, reason, notes);
...
// ═══════════════════════════════════════════════════════════════════
//  RULE 5: Revoke then immediate check — flag is consistently set
//  Stronger than an invariant: proves causality of revoke → isRevoked
// ═══════════════════════════════════════════════════════════════════

rule revokeAndCheckConsistency {
    env e;
    bytes32 agentId;
    uint8 reason;
    string notes;

    // successfully revoke (no revert)
    revokeWithReason(e, agentId, reason, notes);

    // Both views must agree
    assert isRevoked(agentId) == true,
        "isRevoked false after successful revoke";
    assert getRevoked(agentId) == true,
        "getRevoked false after successful revoke";
}
