...
contract AuditLogger is IAuditLogger, AccessControl, Pausable {
    ...
    function logAction(
        ...
        uint256 logIdx = _logIndex[agentId]++;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000021,logIdx)}

        // eventId = keccak256(agentId || actionHash || block.number || logIndex)
        eventId = keccak256(abi.encode(agentId, actionHash, block.number, logIdx));assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000026,eventId)}
    ...
    function getAgentHistory(
        ...
        bytes32[] storage history = _agentHistory[agentId];assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00010022,0)}
        uint256 total = history.length;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000023,total)}
        if (offset >= total) return new bytes32[](0);

        uint256 end = offset + limit;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000024,end)}
        if (end > total) end = total;
        uint256 resultLen = end - offset;assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000025,resultLen)}

        eventIds = new bytes32[](resultLen);assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00020027,0)}
        for (uint256 i; i < resultLen; ++i) {
            eventIds[i] = history[offset + i];bytes32 certora_local40 = eventIds[i];assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000028,certora_local40)}
        }
    }
