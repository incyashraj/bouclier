...
methods {
    // PermissionVault
    function validateUserOp((address,uint256,bytes,bytes,bytes32,uint256,bytes32,bytes,bytes), bytes32) external returns (uint256) envfree;
    function grantPermission(bytes32, (bytes32,address[],bytes4[],address[],uint256,uint256,uint48,uint48,bool,bool,bool,bytes32,uint8,uint8,uint8,uint64), bytes) external;
    function revokePermission(bytes32) external;
    function emergencyRevoke(bytes32) external;
    function getActiveScope(bytes32) external returns (PermissionScope) envfree;
...
rule revokePermissionSetsRevoked {
    ...
    // (getActiveScope returns the stored scope)
    // This verifies that the revoked flag is actually set
    assert getActiveScope(agentId).revoked == true,
...
rule emergencyRevokeSetsBothFlags {
    ...
    require !revReg.isRevoked(agentId);
    require !getActiveScope(agentId).revoked;
...
rule grantPermissionIncreasesNonce {
    ...
    grantPermission(e, agentId, scope, sig);
...
rule expiredScopeFailsValidation {
    ...
    require getActiveScope(agentId).validUntil < e.block.timestamp;
    require getActiveScope(agentId).validUntil != 0;  // scope exists
